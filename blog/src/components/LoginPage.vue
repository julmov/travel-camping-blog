  ...
  <div class="login-page">
    ...
    <form @submit.prevent="handleSubmit">
      <div class="form-group">
        <input type="text" placeholder="Username" id="username" v-model="login" required>
      </div>
      <div class="form-group">
        <input type="password" placeholder="Password" id="password" v-model="password" required>
      </div>
      <p>If you dont have account yet, you can <router-link to="/register">Register</router-link></p>
...
const handleSubmit = async () => {
  try {
    const response = await fetch("https://blog-camping-cbb2c4cfea86.herokuapp.com/users/login", {
      method: 'POST',
      headers: {
